...
namespace L04_FudgeBall {
  ...
  function init(_event: Event): void {
    ...
    document.addEventListener("keypress", (_event: f.EventKeyboard) => {
      if (_event.code == f.KEYBOARD_CODE.E) {
        ...
        if (distance.magnitude > 2.5) return;
  ...
  function createNodeWithComponents(
    _name: string,
    _material: f.Material,
    _mesh: f.Mesh,
    _mass: number,
    _physicsType: f.PHYSICS_TYPE,
    _group: f.PHYSICS_GROUP = f.PHYSICS_GROUP.DEFAULT,
    _colType: f.COLLIDER_TYPE = f.COLLIDER_TYPE.CUBE,
  ...
  function handler_Key_Pressed(_event: KeyboardEvent): void {
    ...
    if (_event.code == f.KEYBOARD_CODE.SPACE) {
      if (isGrounded) playerBody.applyLinearImpulse(new f.Vector3(0, playerJumpForce, 0));
    }
  ...
  function settingUpAJoint(): void {
    environment[10] = createNodeWithComponents(
      "Holder",
      new f.Material("Cube", f.ShaderFlat, new f.CoatColored(new f.Color(0.4, 0.4, 0.4, 1))),
      new f.MeshCube(),
      1,
      f.PHYSICS_TYPE.STATIC,
    ...
    environment[11] = createNodeWithComponents(
      "MovingDrill",
      new f.Material("Cube", f.ShaderFlat, new f.CoatColored(new f.Color(1, 1, 0, 1))),
      new f.MeshCube(),
      1,
      f.PHYSICS_TYPE.DYNAMIC,
      f.PHYSICS_GROUP.GROUP_1
    );
    ...
    let cylindricalJoint: f.ComponentJointCylindrical = new f.ComponentJointCylindrical(
      environment[10].getComponent(f.ComponentRigidbody),
      environment[11].getComponent(f.ComponentRigidbody),
      new f.Vector3(0, 1, 0)
    );
